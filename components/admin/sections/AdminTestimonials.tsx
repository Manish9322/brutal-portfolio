...
const AdminTestimonials: React.FC = () => {
  ...
  const handleSave = () => {
    let newList;
    if (editing === 'new') {
      const newItem = { ...formData, id: Date.now().toString(), order: data.testimonials.length + 1, isFeatured: false } as Testimonial;
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center border-b-4 border-black pb-4">
        <h2 className="text-4xl font-black uppercase tracking-tighter">TESTIMONIALS</h2>
      ...
      <div className="space-y-6">
        {data.testimonials.sort((a,b) => a.order - b.order).map((t) => (
          <div key={t.id} className="border-4 border-black p-6 bg-white hover:bg-gray-50 transition-colors">
