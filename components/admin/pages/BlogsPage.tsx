...
const BlogsPage: React.FC = () => {
  ...
  const handleSave = () => {
    let newList;
    if (editing === 'new') {
      const newItem = { ...formData, id: Date.now().toString(), date: new Date().toISOString().split('T')[0], published: true, slug: (formData.title || '').toLowerCase().replace(/ /g, '-') } as Blog;
      newList = [...data.blogs, newItem];
    } else {
      // Fixed reference error: changed 'i' to 'b' to match the map callback parameter
